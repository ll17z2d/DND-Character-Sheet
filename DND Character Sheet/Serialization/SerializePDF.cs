...
namespace DND_Character_Sheet.Serialization
{
    ...
    public class SerializePDF : ISerializePDF
    {
        ...
        public void Serialize(ICharacterModel character)
        {
            ...
            var uri = new Uri(@"pack://application:,,,/DND Character Sheet;component/Default Documents/Official 5E Character Sheet.pdf", UriKind.RelativeOrAbsolute);
            var fileNameExisting = Application.GetResourceStream(uri);
        ...
        {
            var weaponNotes = new WeaponNotes(new ObservableCollection<WeaponsInventory>(), 
                GetStringFieldOrDefault(form, "AttacksSpellcasting")); //Check if this title is correct

            if (GetStringFieldOrDefault(form, "Wpn Name") != "" && GetStringFieldOrDefault(form, "Wpn1 AtkBonus") != "" && GetStringFieldOrDefault(form, "Wpn1 Damage") != "")
                weaponNotes.WeaponsInventory.Add(new WeaponsInventory(GetStringFieldOrDefault(form, "Wpn Name"),
                GetStringFieldOrDefault(form, "Wpn1 AtkBonus"),
                GetStringFieldOrDefault(form, "Wpn1 Damage")));

            if (GetStringFieldOrDefault(form, "Wpn Name 2") != "" && GetStringFieldOrDefault(form, "Wpn2 AtkBonus ") != "" && GetStringFieldOrDefault(form, "Wpn3 Damage ") != "")
                weaponNotes.WeaponsInventory.Add(new WeaponsInventory(GetStringFieldOrDefault(form, "Wpn Name 2"),
                GetStringFieldOrDefault(form, "Wpn2 AtkBonus "),
                GetStringFieldOrDefault(form, "Wpn2 Damage ")));

            if (GetStringFieldOrDefault(form, "Wpn Name 3") != "" && GetStringFieldOrDefault(form, "Wpn3 AtkBonus  ") != "" && GetStringFieldOrDefault(form, "Wpn3 Damage ") != "")
                weaponNotes.WeaponsInventory.Add(new WeaponsInventory(GetStringFieldOrDefault(form, "Wpn Name 3"),
        ...
        private CharacterNotes GetCharacterNotes(PdfAcroForm form, string filePath) 
            => new CharacterNotes("", //This is where character appearance is (Will try to get this working later)
        ...
        private int GetIntFieldOrDefault(PdfAcroForm form, string fieldName) 
            => int.TryParse(new string(form.GetField(fieldName).GetValueAsString().Replace(" ", "").Where(x => char.IsDigit(x)).ToArray()), out int result) ? result : 0;
        ...
        private void InitialiseDictionary(ICharacterModel character)
        {
            FieldKeyToValueDict = new Dictionary<string, string>() //Think about whether I want to include the spell name & spell desc or just the spell name for serialisation
            {
                ...
                {"HDTotal", character.HPStats.TotalHitDie},
